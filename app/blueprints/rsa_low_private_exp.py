...
rsa_low_private_exp_blueprint = Blueprint("rsa-low-private-exp", __name__)

DATA_FILE = os.path.join("app", "data", "rsa-low-private-exp.json")
...
@rsa_low_private_exp_blueprint.route("/rsa-low-private-exp/<task_id>/params", strict_slashes=False, methods=["GET"])
def params(task_id):
    task_id = str(task_id)
...
@rsa_low_private_exp_blueprint.route("/rsa-low-private-exp/<task_id>/cipher", strict_slashes=False, methods=["GET"])
def cipher(task_id):
    task_id = str(task_id)
    tasks = File.read_json(DATA_FILE, supress_exception=True)
